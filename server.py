...
def _try_save_media_block(block, request_id, msg_idx, block_idx, role):
    ...
    if block.get("type") == "image":
        ...
        if source.get("type") == "base64":
            ...
            return _save_base64(data, mime, request_id, msg_idx, block_idx, role, "image")
...
def convert_anthropic_to_openai(body):
    ...
    if "system" in body:
        ...
        if isinstance(sys_content, list):
            sys_text = " ".join(b.get("text", "") for b in sys_content if b.get("type") == "text")
        else:
    ...
    if "tool_choice" in body:
        ...
        if isinstance(tc, dict):
            if tc.get("type") == "tool":
                openai_body["tool_choice"] = {"type": "function", "function": {"name": tc["name"]}}
...
def convert_anthropic_message(msg):
    ...
    if role == "user":
        ...
        if tool_results:
            ...
            if other_blocks:
                result.append({"role": "user", "content": convert_content_blocks(other_blocks)})
    ...
    if role == "assistant":
        ...
        text_content = " ".join(b.get("text", "") for b in text_blocks) if text_blocks else None
...
def convert_openai_stream_chunk_to_anthropic(chunk, chunk_index, is_first, is_last, model):
...
async def handle_openai_proxy(request):
    ...
    if is_stream:
        return await handle_stream_proxy(
            request, db, session_id, body, headers, backend_url,
            "openai", model, timestamp, start_time, convert_for_client=None
        )
    else:
        return await handle_non_stream_proxy(
            request, db, session_id, body, headers, backend_url,
            "openai", model, timestamp, start_time, convert_response=None
        )
...
async def handle_anthropic_proxy(request):
    ...
    if is_stream:
        return await handle_stream_proxy(
            request, db, session_id, body, headers, backend_url,
            "anthropic", model, timestamp, start_time,
            convert_for_client="anthropic", forward_body=openai_body
        )
    else:
        return await handle_non_stream_proxy(
            request, db, session_id, body, headers, backend_url,
            "anthropic", model, timestamp, start_time,
            convert_response="anthropic", forward_body=openai_body
        )


async def handle_stream_proxy(request, db, session_id, original_body, headers, backend_url,
                              client_type, model, timestamp, start_time,
                              convert_for_client=None, forward_body=None):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(backend_url, json=send_body, headers=headers) as backend_resp:
                        ...
                        for single_line in line_str.split("\n"):
                            ...
                            if single_line.startswith("data: "):
                                ...
                                try:
                                    ...
                                    if convert_for_client == "anthropic":
                                        events = convert_openai_stream_chunk_to_anthropic(
                                            chunk_data, chunk_index,
                                            is_first=(chunk_index == 0),
                                            is_last=bool(chunk_data.get("choices", [{}])[0].get("finish_reason")),
                                            model=model
                                        )
        ...
        async with db.execute(
            """INSERT INTO requests (session_id, timestamp, client_type, model, request_body, response_body, status_code, duration_ms, stream, error)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?)""",
            (session_id, timestamp, client_type, model, request_body_str, response_body_str, status_code, duration_ms, error_msg)
        ) as cursor:
        ...
        for rec in media_records:
            if rec:
                await db.execute(
                    ...
                    (request_id, rec["role"], rec["media_type"], rec["mime_type"], rec["file_path"], rec["message_index"], timestamp)
                )
        await db.commit()
...
async def handle_non_stream_proxy(request, db, session_id, original_body, headers, backend_url,
                                  client_type, model, timestamp, start_time,
                                  convert_response=None, forward_body=None):
            ...
            async with session.post(backend_url, json=send_body, headers=headers) as backend_resp:
                ...
                if status_code == 200:
                    ...
                    if convert_response == "anthropic":
                        client_resp = convert_openai_response_to_anthropic(resp_data, model)
                        final_resp = web.json_response(client_resp)
                    else:
                        final_resp = web.Response(text=resp_text, content_type="application/json")
                else:
                    error_msg = resp_text
                    final_resp = web.Response(text=resp_text, status=status_code, content_type="application/json")
                    resp_data = resp_text
    except Exception as e:
        error_msg = str(e)
    ...
    duration_ms = int((time.time() - start_time) * 1000)
    request_body_str = json.dumps(original_body, ensure_ascii=False)
    response_body_str = json.dumps(resp_data, ensure_ascii=False) if isinstance(resp_data, dict) else resp_data
...
async def api_get_sessions(request):
    ...
    sessions = [
        {"id": r[0], "name": r[1], "created_at": r[2], "last_active": r[3], "request_count": r[4]}
        for r in rows
    ]
...
async def api_get_session_requests(request):
    ...
    for r in rows:
        requests_list.append({
            "id": r[0], "timestamp": r[1], "client_type": r[2], "model": r[3],
            "request_body": json.loads(r[4]) if r[4] else None,
            "response_body": json.loads(r[5]) if r[5] else None,
            "status_code": r[6], "duration_ms": r[7], "stream": bool(r[8]), "error": r[9],
        })
...
async def api_get_request_detail(request):
    ...
    # Get media
    async with db.execute("SELECT id, role, media_type, mime_type, file_path, message_index FROM media_files WHERE request_id = ?", (req_id,)) as cursor:
        media_rows = await cursor.fetchall()

    media = [{"id": m[0], "role": m[1], "media_type": m[2], "mime_type": m[3], "file_path": m[4], "message_index": m[5]} for m in media_rows]

    return web.json_response({
        "id": row[0], "session_id": row[1], "timestamp": row[2], "client_type": row[3],
        "model": row[4],
        "request_body": json.loads(row[5]) if row[5] else None,
        "response_body": json.loads(row[6]) if row[6] else None,
        "status_code": row[7], "duration_ms": row[8], "stream": bool(row[9]), "error": row[10],
        "media": media,
    })
...
async def api_delete_session(request):
    db = request.app["db"]
    session_id = request.match_info["id"]
    await db.execute("DELETE FROM media_files WHERE request_id IN (SELECT id FROM requests WHERE session_id = ?)", (session_id,))
...
async def handle_static(request):
    ...
    return web.Response(status=404, text="Frontend not built. Run: cd frontend && npm run build")
...
async def on_startup(app):
    ...
    print(f"LLM Proxy Sniffer running on http://{config.LISTEN_HOST}:{config.LISTEN_PORT}")
